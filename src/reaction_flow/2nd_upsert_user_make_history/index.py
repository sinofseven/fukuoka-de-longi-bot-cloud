...
def upsert_user_make_history(
    user_id: str, user_name: str, amount: int, table_name: str, dynamodb_resource: ServiceResource
) -> int:
    ...
    option = {
        "Key": {"partitionId": "userMakeHistory", "sortId": now},
...
def create_result(user_name: str, ts: str, amount: int, sack_times: int, user_times: int) -> dict:
    return {"user_name": user_name, "ts": ts, "amount": amount, "sack_times": sack_times, "user_times": user_times}
